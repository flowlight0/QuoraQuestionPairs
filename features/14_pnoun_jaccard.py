...
def pnoun_jaccard(row):
    q1set = set([s for (s, tag) in ast.literal_eval(row['question1']) if tag.startswith('NNP')])
    q2set = set([s for (s, tag) in ast.literal_eval(row['question2']) if tag.startswith('NNP')])

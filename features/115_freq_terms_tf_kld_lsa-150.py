...
#
# def count_vectorizer_with_rare_words(qs):
#     vectorizer1 = CountVectorizer(ngram_range=(1, 1))
#     counter1 = vectorizer1.fit_transform(qs)
#     len(vectorizer1.vocabulary_)
#
#     vocab_freq1 = counter1.sum(axis=0)
#     vocab_freq_map1 = {}
#     vocab_freq_values1 = []
#     for k, v in vectorizer1.vocabulary_.items():
#         vocab_freq_map1[k] = vocab_freq1[0, v]
#         vocab_freq_values1.append([k, vocab_freq1[0, v]])
#     df1 = pd.DataFrame(data=vocab_freq_values1, columns=['word', 'freq'])
#     rare_words = set(df1[df1.freq == 1].word)
#     print(len(rare_words))
#     cleaned_qs = pd.Series([" ".join([w if w not in rare_words else '--rare--' for w in s.split(" ")]) for s in qs])
#     print(len(qs), len(qs[cleaned_qs.str.contains('--rare--')]))
#
#     vectorizer2 = CountVectorizer(ngram_range=(2, 2), min_df=8)
#     counter2 = vectorizer2.fit_transform(cleaned_qs)
#
#     print(len(vectorizer2.vocabulary_))
#     vocab_freq2 = counter2.sum(axis=0)
#     vocab_freq_map2 = {}
#     vocab_freq_values2 = []
#     for k, v in vectorizer2.vocabulary_.items():
#         vocab_freq_map2[k] = vocab_freq2[0, v]
#         vocab_freq_values2.append([k, vocab_freq2[0, v]])

...
def calc_feature(row):
    ...
    c1 = len([word for word in q1.split(' ') if word == 'not' or word.endswith("n't")])
    c2 = len([word for word in q2.split(' ') if word == 'not' or word.endswith("n't")])
    return min(3, abs(c1 - c2))

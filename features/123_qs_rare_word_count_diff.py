...
class FeatureCreator(RowWiseFeatureCreatorBase):
    ...
    def calculate_row_feature(self, row_):
        ...
        q1_sum = sum([word.isalnum() and word not in self.vocab for word in q1])
        q2_sum = sum([word.isalnum() and word not in self.vocab for word in q2])
        return abs(q1_sum - q2_sum)

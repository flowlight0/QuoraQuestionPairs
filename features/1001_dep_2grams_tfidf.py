...
def create_feature(train_path, test_path, n, skip_stopwords):
    ...
    train_feature['dep_2grams_sum_tfidf_q1'] = np.array(train_q1_tfidf.sum(axis=1)).flatten()
    ...
    test_feature['dep_2grams_sum_tfidf_q1'] = np.array(test_q1_tfidf.sum(axis=1)).flatten()
    test_feature['dep_2grams_sum_tfidf_q2'] = np.array(test_q2_tfidf.sum(axis=1)).flatten()
    test_feature['dep_2grams_tfidf_cosine'] = np.array(test_q1_tfidf.multiply(test_q2_tfidf).sum(axis=1)).flatten()

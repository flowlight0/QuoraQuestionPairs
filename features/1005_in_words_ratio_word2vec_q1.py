...
class FeatureCreator(RowWiseFeatureCreatorBase):
    ...
    def calculate_row_feature(self, row_):
        row = row_[1]
        words = str(row['question1']).split()
        return sum(1 for word in words if word in self.model) / len(words)
    ...
    def prepare(self):
        self.model = gensim.models.KeyedVectors.load_word2vec_format('data/input/GoogleNews-vectors-negative300.bin', binary=True)

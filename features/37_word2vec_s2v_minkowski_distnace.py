...
import numpy as np
import pandas as pd
from scipy.spatial.distance import cosine, jaccard, canberra, euclidean, minkowski
...
def create_feature(data_file):
    ...
    df[column_name] = np.nan_to_num([minkowski(x, y, 3) for (x, y) in zip(np.nan_to_num(question1_vectors),
